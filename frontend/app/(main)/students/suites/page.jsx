...
import { Plus, FlaskConical, Trash2, Eye, X, Pencil } from "lucide-react";
import { API_BASE } from "@/lib/apiBase";
import { useAuth } from "@/contexts/AuthContext";
import Dialog from "@/components/Dialog";

export default function StudentTestSuitesPage() {
    const { user } = useAuth();
    ...
    const [newCase, setNewCase] = useState({ input: "", expectedOutput: "", label: "" });
    ...
    const handleOpenSuite = async (suiteId) => {
        ...
        setAddingCase(false);
        setNewCase({ input: "", expectedOutput: "", label: "" });
    ...
    const handleAddCase = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE}/testsuite/${openSuiteId}/case`, {
                method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...newCase, hidden: false }),
            });
    ...
    return (
        <div className="p-8">
            <div className="max-w-4xl mx-auto">
                <div className="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <p className="text-zinc-500 dark:text-zinc-400 text-sm mt-1">Save reusable test cases to load quickly when testing your assignments.</p>
                    </div>
                ...
                {suites.length === 0 ? (
                    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl p-12 text-center shadow-sm">
                        ...
                        <p className="text-zinc-500 dark:text-zinc-400 text-sm mt-1">Create a suite to save and reuse test cases across your assignments.</p>
                        ...
                        <div className="flex-1 overflow-auto p-6 space-y-4">
                            ...
                            {addingCase && (
                                <div className="p-4 bg-zinc-50 dark:bg-zinc-800 border rounded-xl space-y-3 mt-2" style={{ borderColor: "#86263366" }}>
                                    ...
                                    <div className="flex gap-2">
                                        <button type="button" onClick={() => { setAddingCase(false); setNewCase({ input: "", expectedOutput: "", label: "" }); }} className="flex-1 py-2 text-sm font-medium text-zinc-600 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-700 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-600 transition-colors">Cancel</button>
            ...
            <Dialog isOpen={deleteConfirm.isOpen} onClose={() => setDeleteConfirm({ isOpen: false, suite: null })} title="Delete Suite" size="sm">
                <div className="space-y-4">
                    <p className="text-zinc-600 dark:text-zinc-300">Are you sure you want to delete <span className="font-semibold text-zinc-900 dark:text-white">{deleteConfirm.suite?.name}</span>? This will permanently remove all test cases in the suite.</p>
                    <div className="flex gap-3 pt-2">
